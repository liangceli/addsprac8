...
void Heap<T>::remove(T value) {
  ...
  values.erase(item);
  //替换之后，将被替换之后的元素和它的子元素中最小的那一个进行调换
  T leftchild=values.at(leftchild_idx);
  T rightchild=values.at(rightchild_idx);
  int smallestchild_idx;
  //要么有两个children 要么有一个child（左）
  if(rightchild_idx>values.size()){
    T new_temp=values.at(leftchild_idx);
     values.at(leftchild_idx)=values.at(idx);
    values.at(idx)=new_temp;
  }else{
    if(leftchild>rightchild){
      T new_temp=values.at(rightchild_idx);
       values.at(rightchild_idx)=values.at(idx);
       values.at(idx)=new_temp;
